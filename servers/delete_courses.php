...
// } else {
    ...
    if (($course != '...')) {
        if ($check_course_array) {
            $delete_course = mysqli_query($connectpage, "DELETE FROM $tablename WHERE course = '$course' ");
            if ($delete_course) {
                echo 'course successfully deleted<br>';
                $check_course1 = mysqli_query($connectpage, "SELECT * FROM $tablename");
                // $check_course_array = mysqli_fetch_assoc($check_course1);
                $counter = 1;
                while ($all_courses = mysqli_fetch_assoc($check_course1)){
                ...
                echo 'unable to delete course<br>';
            }
        
        }else {
            // echo 'please select a course<br>';
            echo $course.' is not previously registered, pls add course';
        }
    }else {
        // echo 'course not previously added, pls click add to do so';
        echo 'your are yet to select a course<br>';   
    }
